...
const Icon = {
  ...
  work: (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <rect x="3" y="7" width="18" height="13" rx="2"/><path d="M16 7V5a3 3 0 0 0-3-3h-2a3 3 0 0 0-3 3v2"/></svg>
  ),
  about: (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="12" cy="7" r="4"/><path d="M5.5 21a7.5 7.5 0 0 1 13 0"/></svg>
  ),
  contact: (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M21 15a4 4 0 0 1-4 4H7l-4 4V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4z"/></svg>
  ),
};
...
export function MobileNav() {
  ...
  const sectionIds = useMemo(() => items.map((n) => n.href.replace('#', '')), []);
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        const current = positions.filter((p) => p.top <= 0).sort((a, b) => b.top - a.top)[0];
          ...
          return (
            <li key={item.href} className="">
              <Link
                ...
                className={'flex flex-col items-center justify-center gap-1 rounded-md px-2 py-2 text-[11px] font-medium ' + (isActive ? 'text-brand-500 bg-brand-500/10' : 'text-gray-600 dark:text-gray-300')}
              >
                <span className={isActive ? 'text-brand-500' : 'text-gray-500 dark:text-gray-400'}>{item.icon}</span>
