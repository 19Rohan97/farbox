...
export function CaseStudies({ caseStudies }: { caseStudies?: typeof site.caseStudies }) {
  ...
  return (
    <section id="case-studies" className="relative overflow-hidden py-24 border-t border-gray-100 dark:border-gray-800">
      ...
      <Container>
        ...
        {cs.map((study, idx) => {
          ...
          return (
            <motion.div key={study.slug} variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} className="mt-12 grid items-center gap-10 lg:grid-cols-2">
              <motion.div variants={item} className={(reverse ? 'lg:order-2 ' : 'lg:order-1 ') + 'relative'}>
                <ShapesBackdrop />
                <motion.div initial={{ opacity: 0, scale: 0.98 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, ease: 'easeOut' }} viewport={{ once: true, amount: 0.3 }} className="relative overflow-hidden rounded-3xl border border-gray-200 shadow-sm dark:border-gray-800">
              ...
              <motion.div variants={item} className={reverse ? 'lg:order-1' : 'lg:order-2'}>
                <div className="max-w-xl">
                  <div className="chip">{study.category}</div>
                  <h3 className="mt-3 text-3xl font-semibold leading-tight tracking-tight sm:text-4xl">{study.title}</h3>
                  <p className="mt-4 text-gray-700 dark:text-gray-300">{study.summary}</p>
                  <ul className="mt-6 grid gap-3 sm:grid-cols-2">
                    {study.results.map((r) => (
                      <li key={r} className="inline-flex items-center gap-2 rounded-lg border border-brand-500/30 bg-brand-500/10 px-3 py-2 text-sm text-brand-600 dark:text-brand-500">
                  ...
                  <div className="mt-7 flex flex-wrap gap-3">
                    <a href={study.link} className="btn-primary">Read case study</a>
