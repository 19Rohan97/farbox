...
export default function BlogPost({ params }: Props) {
  ...
  const prev = idx >= 0 && idx + 1 < all.length ? { slug: all[idx + 1].slug, title: all[idx + 1].title } : undefined;
  const next = idx > 0 ? { slug: all[idx - 1].slug, title: all[idx - 1].title } : undefined;

  return (
    <main className="bg-white text-gray-900 dark:bg-neutral-950 dark:text-gray-100">
      <Script id="jsonld-article" type="application/ld+json" strategy="afterInteractive" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonld) }} />
      <article>
        <header className="pt-10 sm:pt-12">
          <Container>
            <div className="relative overflow-hidden rounded-3xl border border-brand-500/30 bg-white px-6 py-8 shadow-md sm:p-10 dark:bg-neutral-950">
              ...
              <ShapesBackdrop />
              <nav aria-label="Breadcrumb" className="text-xs text-gray-600 dark:text-gray-400">
                <ol className="flex items-center gap-2">
                  <li><Link href="/" className="hover:text-gray-900 dark:hover:text-gray-200">Home</Link></li>
                  <li aria-hidden>›</li>
                  <li><Link href="/blog" className="hover:text-gray-900 dark:hover:text-gray-200">Blog</Link></li>
                  <li aria-hidden>›</li>
                  <li className="text-gray-900 dark:text-gray-100 truncate max-w-[60vw] sm:max-w-none">{post.title}</li>
                </ol>
              </nav>
            ...
            <div className="grid gap-8 lg:grid-cols-12">
              ...
              <aside className="lg:col-span-4">
                <div className="sticky top-24 space-y-6">
                      ...
                      <ul className="mt-3 space-y-2 text-sm">
                        {toc.map((h, i) => (
                          <li key={i} className={h.level === 3 ? 'ml-3' : ''}>
                            <a href={`#${h.id}`} className="text-gray-700 hover:text-brand-500 dark:text-gray-200">{h.text}</a>
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {(() => {
                    const recent = getAllPosts().filter((p) => p.slug !== post.slug).slice(0, 5);
                        ...
                        <ul className="mt-3 space-y-2 text-sm">
                          {recent.map((r) => (
                            <li key={r.slug} className="flex items-start justify-between gap-3">
                              <Link href={`/blog/${r.slug}`} className="text-gray-800 hover:underline dark:text-gray-200">
                                {r.title}
                              </Link>
                              <span className="shrink-0 text-xs text-gray-500 dark:text-gray-400">{new Date(r.date).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/5">
                    ...
                    <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">Like what you’re reading? Book a quick call or reach out.</p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      <Link href="#contact" className="btn-secondary">Contact</Link>
                      <Link href="#call" className="btn-primary">Book a call</Link>
                    </div>
                  </div>
                </div>
              </aside>
