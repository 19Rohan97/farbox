...
export const metadata: Metadata = {
  title: 'Blog',
  description: 'Insights on design, development, and product from Farbox.',
};

function estimateReadMinutes(post: { content?: string[]; contentMd?: string }): number {
  let text = '';
  if (post.contentMd && typeof post.contentMd === 'string') text = post.contentMd;
    ...
    <main className="bg-white text-gray-900 dark:bg-neutral-950 dark:text-gray-100">
      <section className="relative overflow-hidden pb-10 pt-16 sm:pt-20">
        ...
        <Container>
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-5xl">Blog</h1>
          <p className="mt-3 max-w-2xl text-gray-600 dark:text-gray-300">Notes on building, shipping, and scaling great sites and products.</p>
        </Container>
      </section>
      <section className="pb-16">
        <Container>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {posts.map((post) => (
              <article key={post.slug} className="card overflow-hidden transition hover:shadow-md flex h-full flex-col">
                {post.coverImage && (
                ...
                <div className="p-4 flex flex-1 flex-col">
                  ...
                  <div className="mt-auto pt-4">
                    <Link href={`/blog/${post.slug}`} className="btn-primary">Read more</Link>
                  </div>
                </div>
              </article>
            ))}
